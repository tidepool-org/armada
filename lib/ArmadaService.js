...
function createServer(config){
    ...
    var user = require('./routes/user')(config,'http://localhost:10004');

    //health check
    server.get('/api/group/status',groups.status);

    //user membership
    server.get('/api/group/membership/:userid/member', user.checkToken, user.getToken, groups.memberOf);

    //updating
    server.post('/api/group', user.checkToken, user.getToken, groups.addGroup);

    server.post('/api/group/:groupid/user', user.checkToken, user.getToken, groups.addToGroup);
    server.del('/api/group/:groupid/user', user.checkToken, user.getToken, groups.removeFromGroup);
    
    //group members
    server.get('/api/group/:groupid/members', user.checkToken, user.getToken, user.getAllUsersForGroup);
