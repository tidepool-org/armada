...
module.exports = function mongoHandler(connectionString) {
  ...
  */
  function isDown(status){
    status.deps.up = _.without(dependencyStatus.deps.up, 'mongo');
    status.deps.down = _.union(dependencyStatus.deps.down, ['mongo']);
    return status;
  }
  ...
  */
  function isUp(status){
    status.deps.down = _.without(dependencyStatus.deps.down, 'mongo');
    status.deps.up = _.union(dependencyStatus.deps.up, ['mongo']);
    return status;
  }
